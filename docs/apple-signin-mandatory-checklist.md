...
- [ ] 未ログイン時の起動導線を`/(onboarding)/welcome`へ変更（`/login`固定を撤去）
- [ ] タブ/深いリンクで未ログイン時はオンボーディングへ誘導
- [ ] オンボーディングWelcomeの「ログインに戻る」導線を整理（未ログイン前提）

## フェーズ2: オンボーディング完了後にApple必須画面へ

- [ ] `analysis`でサーバー保存をしない（ゲストでプラン生成→次へ）
- [ ] `analysis`の次アクションを`apple-connect`へ固定（スキップ不可前提）
